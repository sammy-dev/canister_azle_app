...
struct StableBTreeMap0ValueType(Users);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap1ValueType(EducationHistory);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap2ValueType(WorkHistory);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap3ValueType(Company);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap4ValueType(Job);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
struct StableBTreeMap5ValueType(JobApplication);
impl CdkActTryIntoVmValue<&mut boa_engine::Context<'_>, boa_engine::JsValue>
...
#[ic_cdk_macros::query(name = "getWorkHistory")]
#[candid::candid_method(query, rename = "getWorkHistory")]
async fn _cdk_user_defined_getWorkHistory() -> (_AzleResult<Vec<WorkHistory>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getWorkHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
...
#[ic_cdk_macros::query(name = "getEducationHistory")]
#[candid::candid_method(query, rename = "getEducationHistory")]
async fn _cdk_user_defined_getEducationHistory() -> (_AzleResult<
    Vec<EducationHistory>,
    String,
>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getEducationHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getEducationHistory", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getEducationHistory"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::query(name = "getJobPostList")]
#[candid::candid_method(query, rename = "getJobPostList")]
async fn _cdk_user_defined_getJobPostList() -> (_AzleResult<Vec<Job>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getJobPostList".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getJobPostList", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getJobPostList"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(&boa_engine::JsValue::Null, &[], &mut boa_context)?;
                let final_return_value = async_await_result_handler(
...
#[ic_cdk_macros::query(name = "getJobPostListByCompany")]
#[candid::candid_method(query, rename = "getJobPostListByCompany")]
async fn _cdk_user_defined_getJobPostListByCompany(
    _cdk_user_defined_companyId: String,
) -> (_AzleResult<Vec<Job>, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "getJobPostListByCompany".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("getJobPostListByCompany", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "getJobPostListByCompany"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
                        &[
                            _cdk_user_defined_companyId
                                .try_into_vm_value(&mut boa_context)?,
                        ],
                        &mut boa_context,
                    )?;
...
#[ic_cdk_macros::update(name = "userRegister")]
#[candid::candid_method(update, rename = "userRegister")]
async fn _cdk_user_defined_userRegister(
    _cdk_user_defined_payload: UsersPayload,
) -> (_AzleResult<Users, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "userRegister".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("userRegister", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "userRegister"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "addWorkHistory")]
#[candid::candid_method(update, rename = "addWorkHistory")]
async fn _cdk_user_defined_addWorkHistory(
    _cdk_user_defined_payload: WorkHistoryPayload,
) -> (_AzleResult<WorkHistory, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "addWorkHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("addWorkHistory", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "addWorkHistory"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateWorkHistory")]
#[candid::candid_method(update, rename = "updateWorkHistory")]
async fn _cdk_user_defined_updateWorkHistory(
        ...
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateWorkHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateWorkHistory", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateWorkHistory"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "removeWorkHistory")]
#[candid::candid_method(update, rename = "removeWorkHistory")]
async fn _cdk_user_defined_removeWorkHistory(
    _cdk_user_defined_id: String,
) -> (_AzleResult<WorkHistory, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "removeWorkHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("removeWorkHistory", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "removeWorkHistory"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "addEducationHistory")]
#[candid::candid_method(update, rename = "addEducationHistory")]
async fn _cdk_user_defined_addEducationHistory(
    _cdk_user_defined_payload: EducationHistoryPayload,
) -> (_AzleResult<EducationHistory, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "addEducationHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("addEducationHistory", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "addEducationHistory"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateEducationHistory")]
#[candid::candid_method(update, rename = "updateEducationHistory")]
async fn _cdk_user_defined_updateEducationHistory(
    _cdk_user_defined_id: String,
    _cdk_user_defined_payload: EducationHistoryPayload,
) -> (_AzleResult<EducationHistory, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateEducationHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateEducationHistory", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateEducationHistory"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "removeEducationHistory")]
#[candid::candid_method(update, rename = "removeEducationHistory")]
async fn _cdk_user_defined_removeEducationHistory(
    _cdk_user_defined_id: String,
) -> (_AzleResult<EducationHistory, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "removeEducationHistory".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("removeEducationHistory", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "removeEducationHistory"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "companyRegister")]
#[candid::candid_method(update, rename = "companyRegister")]
async fn _cdk_user_defined_companyRegister(
    _cdk_user_defined_payload: CompanyPayload,
) -> (_AzleResult<Company, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "companyRegister".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("companyRegister", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "companyRegister"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateCompany")]
#[candid::candid_method(update, rename = "updateCompany")]
async fn _cdk_user_defined_updateCompany(
    _cdk_user_defined_id: String,
    _cdk_user_defined_payload: CompanyPayload,
) -> (_AzleResult<Company, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateCompany".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateCompany", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateCompany"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "removeCompany")]
#[candid::candid_method(update, rename = "removeCompany")]
async fn _cdk_user_defined_removeCompany(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Company, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "removeCompany".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("removeCompany", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "removeCompany"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "addJobPost")]
#[candid::candid_method(update, rename = "addJobPost")]
async fn _cdk_user_defined_addJobPost(
    _cdk_user_defined_payload: JobPayload,
) -> (_AzleResult<Job, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "addJobPost".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("addJobPost", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "addJobPost"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "updateJobPost")]
#[candid::candid_method(update, rename = "updateJobPost")]
async fn _cdk_user_defined_updateJobPost(
    _cdk_user_defined_id: String,
    _cdk_user_defined_payload: JobPayload,
) -> (_AzleResult<Job, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "updateJobPost".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("updateJobPost", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "updateJobPost"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "removeJobPost")]
#[candid::candid_method(update, rename = "removeJobPost")]
async fn _cdk_user_defined_removeJobPost(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Job, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "removeJobPost".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("removeJobPost", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "removeJobPost"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "closeJobPost")]
#[candid::candid_method(update, rename = "closeJobPost")]
async fn _cdk_user_defined_closeJobPost(
    _cdk_user_defined_id: String,
) -> (_AzleResult<Job, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "closeJobPost".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("closeJobPost", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "closeJobPost"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
#[ic_cdk_macros::update(name = "applyJobPost")]
#[candid::candid_method(update, rename = "applyJobPost")]
async fn _cdk_user_defined_applyJobPost(
    _cdk_user_defined_payload: JobApplicationPayload,
) -> (_AzleResult<JobApplication, String>) {
    unwrap_or_trap(|| {
        BOA_CONTEXT_REF_CELL
            .with(|boa_context_ref_cell| {
                let mut boa_context = boa_context_ref_cell.borrow_mut();
                ...
                METHOD_NAME_REF_CELL
                    .with(|method_name_ref_cell| {
                        let mut method_name_mut = method_name_ref_cell.borrow_mut();
                        *method_name_mut = "applyJobPost".to_string();
                    });
                MANUAL_REF_CELL
                    .with(|manual_ref_cell| {
                        let mut manual_mut = manual_ref_cell.borrow_mut();
                ...
                let function_js_value = exports_js_object
                    .get("applyJobPost", &mut boa_context)?;
                let function_js_object = function_js_value
                    .as_object()
                    .ok_or_else(|| RuntimeError::ReferenceError(
                        format!("{} is not defined", "applyJobPost"),
                    ))?;
                let boa_return_value = function_js_object
                    .call(
                        &boa_engine::JsValue::Null,
...
struct EducationHistory {
    id: Box<String>,
    userId: Box<String>,
    educationLevel: Box<String>,
    institutionName: Box<String>,
    fieldOfStudy: Box<String>,
    yearOfEntry: Box<u64>,
    yearOfGraduation: Box<u64>,
    gpa: Box<_CdkFloat64>,
...
struct Job {
    id: Box<String>,
    companyId: Box<String>,
    position: Box<String>,
    requirements: Box<String>,
    location: Box<String>,
    salary: Box<String>,
    description: Box<String>,
    postOwner: Box<String>,
    postStatus: Box<bool>,
    createdAt: Box<u64>,
...
struct JobApplication {
    ...
    jobId: Box<String>,
    userId: Box<String>,
    coverLetter: Box<String>,
    resume: Box<String>,
    portofolio: Box<Option<String>>,
    applicationDate: Box<u64>,
}
#[derive(
    serde::Deserialize,
    Debug,
